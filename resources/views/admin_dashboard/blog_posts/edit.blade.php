@extends('layouts.admin_layout', ['title' => 'Edit Blog Post'])
...
<div class="p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Update {{ $blogPost->title }}.</p>
    </div>

    <form method="POST" action="{{ route('dashboard.blog-posts.update', $blogPost) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        @include('admin_dashboard.blog_posts._form', ['submitLabel' => 'Save Changes'])
    </form>
</div>
@endsection
